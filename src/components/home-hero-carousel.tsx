...
const SLIDES = [
  ...
  },
  {
    ...
    subtitle: "Free shipping on orders over $300. 2-year warranty on every SkyGear drone.",
    cta: "View deals",
    to: "/shop" as const,
  ...
  return (
    <section className="relative overflow-hidden bg-navy">
      <Carousel setApi={setApi} opts={{ loop: true }} className="w-full">
            ...
            <CarouselItem key={slide.title} className="basis-full pl-0">
              <div className="relative min-h-[420px] sm:min-h-[480px] md:min-h-[560px]">
                ...
                <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-center px-4 py-20 sm:py-28 md:py-32">
                  ...
                  <h1 className="mt-3 max-w-2xl font-display text-3xl font-extrabold leading-tight text-white sm:text-5xl md:text-6xl">
                    {slide.title}
                  </h1>
                  <p className="mt-4 max-w-xl text-sm text-white/85 sm:text-base">{slide.subtitle}</p>
                  <Link
                    to={slide.to}
        ...
        <CarouselPrevious className="left-3 top-1/2 border-white/20 bg-navy/60 text-white hover:bg-navy/80 sm:left-5" />
        <CarouselNext className="right-3 top-1/2 border-white/20 bg-navy/60 text-white hover:bg-navy/80 sm:right-5" />
      </Carousel>
        ...
        {SLIDES.map((_, i) => (
          <button
            ...
            className={cn(
              ...
              current === i ? "w-8 bg-primary" : "w-2 bg-white/50 hover:bg-white/80",
            )}
          />
        ))}
      </div>
